...
if (isset($_POST['search'])) {
    ...
    $selected_alarm = $_POST['selected_alarm'];

    
    $alarm_id = "SELECT ALARM_ID FROM ALARM_CATEGORY WHERE ALARM_NAME = '$selected_alarm' ";
    // echo $selected_sensor;
    $result = mysqli_query($conn, $alarm_id);
    if ($result && mysqli_num_rows($result) > 0) {
        $a_id = mysqli_fetch_assoc($result);
        $alarm_id = $a_id['ALARM_ID'];
        // echo "$sensor_id";
        
    }
   
    $query = findAlarmMessageBasedOnSpecificAlarmBetweenOnTwoDates($alarm_id,$start_date,$end_date);
                        ...
                        <form method="POST">
                            <div class="row">
                                ...
                                <div class="col-md-4 col-lg-2 "
                                  ...
                                  //Gather Information
                                  $sql_for_selected_alarm = "SELECT * FROM ALARM_CATEGORY WHERE ALARM_CATEGORY.DELETE_FLAG = 0 ORDER BY ALARM_CATEGORY.ALARM_ID ASC";
                                  $result_for_selected_alarm = mysqli_query($conn, $sql_for_selected_alarm);

                                  ?>

                                    <select class="form-select" aria-label="Select Specific Alarm" id="selected_alarm"
                                        name="selected_alarm">

                                        <option value='<?php echo $selected_alarm ?>'><?php echo $selected_alarm ?>
                                        </option>

                                        <!-- Loop For Fetch Result -->
                                        <?php while($row = mysqlI_fetch_array($result_for_selected_alarm) ) : ?>
                                        <option value=<?php echo($row['ALARM_NAME']);?>>
                                            <?php echo($row['ALARM_NAME']);?></option>
...
if ($result && mysqli_num_rows($result) > 0) {
    while ($list = mysqli_fetch_assoc($result)) {
        $ALARM_NAME = $list['ALARM_NAME'];
        $ALARM_MESSAGE = $list['ALARM_MESSAGE'];
        $ALARM_RECORDED_DATE = $list['ALARM_RECORDED_DATE'];
    ...
    echo " <a href='showAlarmMessageBasedOnSpecificAlarmBetweenTwoDate.php' style='float:right;margin:1%' class='btn btn-danger'>Reset</a>";
...
<a href='showAlarmMessageBasedOnSpecificAlarmBetweenTwoDate.php' style='float:right;margin:1%' class='btn btn-danger'>Search Again</a>
";
  } 
